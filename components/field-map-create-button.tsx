...
const SocialLink = z.object({
  platform: z.string().min(1, "Plataforma obrigatória").trim(),
  url: z.string().url("URL inválido").trim(),
});

const Schema = z.object({
  fieldMap: z.string().min(1, "Nome é obrigatório").trim(),
...
export default function FieldMapCreateButton() {
  ...
  async function onSubmit(values: Values) {
    try {
      ...
      toast.success("Field map criado", { description: values.fieldMap });
      setOpen(false);
      form.reset();
      router.refresh();
    } catch (e: any) {
      toast.error("Falha ao criar", { description: e?.message || "Erro desconhecido" });
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Novo field map</DialogTitle>
          <DialogDescription>Insere os detalhes do campo.</DialogDescription>
        </DialogHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="sm:col-span-2">
                  ...
                  <FormControl><Input placeholder="Ex.: Campo Alfa" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="sm:col-span-2">
                  ...
                  <FormControl><Input placeholder="Cidade, GPS, etc." {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="sm:col-span-2">
                  ...
                  <FormControl><Textarea rows={3} placeholder="Notas sobre o campo..." {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
                  ...
                  {fields.map((f, i) => (
                    <div key={f.id} className="grid gap-2 sm:grid-cols-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormControl><Input placeholder="Plataforma (ex.: Instagram)" {...field} /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
